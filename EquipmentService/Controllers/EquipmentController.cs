        ...
        public async Task<IEnumerable<EquipmentRequestDto>> GetEquipmentRequests()
        {
            ...
            using (var httpClient = new HttpClient()) { 
                ...
                foreach (EquipmentRequest request in requests)
                {
                    ...
                    var response = await httpClient.GetAsync($"http://52.191.231.66/api/Team/{request.TeamID}");
                    string apiResponse = await response.Content.ReadAsStringAsync();
                    var team = JsonConvert.DeserializeObject<TeamDto>(apiResponse);
                    convert.Team = team;

                    response = await httpClient.GetAsync($"http://40.76.159.244/api/User/{request.UserID}");
